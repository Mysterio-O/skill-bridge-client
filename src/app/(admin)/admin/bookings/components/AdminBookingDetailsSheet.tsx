...
export default function AdminBookingDetailsSheet({
    booking,
    onClose,
    onBookingUpdated,
}: {
    ...
    async function setStatus(status: string) {
        ...
        const t = toast({ title: "Please wait...", description: "Updating..." });
        ...
        try {
            ...
            t.update({ title: "Updated", description: `Status changed to ${status}.` });
        } catch (e) {
            t.update({
                title: "Update failed",
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            {/* Admin: quick status change (adjust to your enum) */}
                            {
                                booking.status === 'pending' ? (
                                    ...
                                    <Button variant="outline" disabled={busy} onClick={() => setStatus("completed")}>
                                        {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
                                        Complete
                                    </Button>
                                )
